...
```html
<div class="tablist">
  <div role="tablist" aria-orientation="horizontal|vertical">
    <button
      type="button"
      class="btn-collapse"
      id="unstyled-tabs-1-title"
      aria-selected="true"
  ...
  <div
    id="unstyled-tabs-1"
    role="tabpanel"
    aria-labelledby="unstyled-tabs-1-title"
  >
  ...
  <div
    ...
    class="hidden"
    role="tabpanel"
    aria-labelledby="unstyled-tabs-2-title"
  >
  ...
  <div
    ...
    class="hidden"
    role="tabpanel"
    aria-labelledby="unstyled-tabs-3-title"
  >
...
>
  <div class="modal-dialog" role="document">
    <div class="modal-content card max-w-2xl rounded-2xl">
      <div class="modal-header">
        <h3 id="exampleModalLabel" class="modal-title">عنوان مودال</h3>
        <button type="button" class="btn-close" aria-label="بستن مودال">
          &times
        </button>
      </div>
...
<button
  type="button"
  data-pjstack="#simpleDrawer"
  aria-haspopup="dialog"
  aria-controls="simpleDrawer"
>
  open
</button>
...
<div
  ...
  tabindex="-1"
  role="dialog"
  aria-labelledby="simpleDrawerLabel"
  aria-hidden="true"
  aria-modal="true"
>
  ...
  <div class="drawer-body">
    ...
    accusantium id optio voluptates veritatis aliquid quibusdam ut.
  </div>
</div>
```
...
```javascript
pjstack.open(element); //open element
pjstack.close(element); //close element
pjstack.showToast(content, (type = "info")); //Display Toast

//On show or hide trigger events
element.addEventListener("pjstack:show", (e) => {
  console.log(e.detail.trigger);
});
element.addEventListener("pjstack:hide", (e) => {
  console.log(e.detail.trigger);
});
```
...
$(document).on("submit", "form.arvand-contactform", function (e) {
  ...
  $.post(
    ajaxURL,
    { action: "arvand_contact_form", form_data: params },
    function (resp) {
      if (resp.success) {
...
$(document).on("click", "[data-ilikeit] button", function (e) {
  ...
  $.post(
    ajaxURL,
    { action: "entry_user_reaction", params: params },
    function (resp) {
      if (resp.success) {
...
document.querySelectorAll("[data-scrollspy]").forEach((element) => {
  const wrapper = document.querySelector(element.dataset.scrollspy);
  if (!wrapper) {
    console.error(
      "Scrollspy wrapper not found for:",
      element.dataset.scrollspy,
    );
    return;
  }
  ...
  if (firstTarget) spygap = firstTarget.offsetTop - wrapper.offsetHeight;
  ...
  window.addEventListener(
    ...
    () => {
      ...
      window.requestAnimationFrame(() => {
        ...
        const foundSection = sectionData.find(
          (s) => y >= s.top && y <= s.bottom,
        );
        const match =
          y <= spygap
            ? firstItem
        ...
        if (match !== activeLink) {
          ...
          element.scrollLeft = match.offsetLeft - element.offsetLeft;
          activeLink = match;
        }
        ticking = false;
      });
...
document.querySelectorAll(".toggle-password").forEach((btn) => {
  ...
  if (!selector) return;
  const isToggle = btn.matches("[type=checkbox], [type=radio]");
  const event = isToggle ? "change" : "click";
  const toggle = () => {
    const show = isToggle ? btn.checked : !btn.classList.contains("show");
    document.querySelectorAll(selector).forEach((input) => {
      input.type = show ? "text" : "password";
    });
    btn.classList.toggle("show", show);
  };
  btn.addEventListener(event, toggle);
});
```
...
document.querySelectorAll("[data-tooltip]").forEach((el) => {
  ...
  el.addEventListener("mouseenter", () => {
    tip = Object.assign(document.createElement("span"), {
      className: "tooltip",
      textContent: el.dataset.tooltip,
    });
    document.body.appendChild(tip);
    if (el.dataset.tooltipClass) {
      el.dataset.tooltipClass
        .split(" ")
        .forEach((c) => c.trim() && tip.classList.add(c));
    }
    const r = el.getBoundingClientRect();
    const tr = tip.getBoundingClientRect();
    tip.style.cssText = `top:${r.top - tr.height - 8 + scrollY}px;left:${r.left + r.width / 2 - tr.width / 2}px;`;
    setTimeout(() => (tip.style.opacity = "1"), 10);
  });

  el.addEventListener("mouseleave", () => {
    tip && ((tip.style.opacity = "0"), setTimeout(() => tip.remove(), 200));
  });
});
```
...
```javascript
document.querySelectorAll(".limit-maxheight").forEach(function (container) {
  const content = container.querySelector(".content");
  const btn = container.querySelector(".more-button");

  let maxH = parseFloat(getComputedStyle(content).maxHeight);
  if (isNaN(maxH)) maxH = 150;
  if (content.scrollHeight > maxH + 1) {
    btn.disabled = false;
    btn.onclick = () => {
...
```css
.tooltip {
  @apply absolute opacity-0 z-9999 text-card bg-foreground rounded-lg py-1.5 px-2 text-xs transition-opacity;
  &:after {
    content: "";
    top: 100%;
    left: 50%;
